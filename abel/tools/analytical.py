...
def scaling(angle, amp=0.1, phase=2):
    # define a simple scaling that will squish a circle into a "flower"
    return 1 + amp*(np.sin(phase*angle)**4)


def flower_distort(IM, amp=0.1, phase=2):
    """ Distort image with a sinusoidal radial scaling.  Intensity `*R**2`
        Squish a circle into a "flower".
    ...
    IM : numpy 2D array
        image
    amp: float
        amplitude of the sinusoidal radial perturbation
    phase: float
        phase of the sinusoidal perturbation

    Returns
    -------
    IMdistort: numpy 2D array
        distorted image
    ...
    IM *= R**2 # making the sample image more beautiful...

    R_rescaled = R*scaling(T, amp, phase) # angle-dependent scaling

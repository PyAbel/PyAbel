...
def test_two_point_basis_sets_cache():
    n = 121
    file_name = os.path.join(DATA_DIR, "two_point_basis_{}_{}.npy".format(n, n))
    if os.path.exists(file_name):
        os.remove(file_name)
    # 1st call generate and save
    abel.tools.basis.get_bs_cached("two_point", n, basis_dir=DATA_DIR, 
                                verbose=False)
...
def test_two_point_shape():
    n = 21
    x = np.ones((n, n), dtype='float32')

    recon = abel.two_point.two_point_transform(x, direction='inverse')

    assert recon.shape == (n, n) 


def test_two_point_zeros():
    n = 21
...
def test_two_point_inverse_transform_gaussian():
    """Check two_point inverse transform with a Gaussian function"""
    n = 501   
    r_max = 251
...
def test_two_point_1d_gaussian(n=101):
...
def test_two_point_2d_gaussian(n=101):
    ...
    AQ0 = abel.two_point.two_point_transform(Q0)
...
if __name__ == "__main__":
    test_two_point_shape()
    test_two_point_zeros()
    test_two_point_1d_gaussian()
    test_two_point_inverse_transform_gaussian()
    test_two_point_2d_gaussian()

...
def init_abel(xc,yc):
    ...
    for ii in range(0,xc+1): 
        for jj in range(ii, xc+1):
            val1[ii,jj] = np.arcsin((ii+1)/float(jj+1))-np.arcsin((ii)/float(jj+1)) 
...
def iabel_onion_peeling(IM,sym_lr=False,sym_ud=False):
    ...
    if w%2==1:  raise ValueError('Image width must be even')

    if np.any(IM<0):
        print 'Image cannot have negative values, setting negative values to zero'
        IM[IM<0] = 0 # cannot have negative values

    # calculate val1 and val2, which are 2D arrays of what appear to be scaling factors
    val1,val2 = init_abel(w,h) 

    abel_arr = IM*0 # initialize 2D array for final transform
    rest_arr = IM   # initialize 2D array that will be manipulated during the transform
    vect = np.zeros(h) # initialize a 1D array that is temorarily used to store scaling factors


    for col_index in range(0,w): # iterate over the columns (x-values) in the slice space 

        #if col_index%100 == 0: print col_index

        idist = w - col_index # this is basically the x-coordinate. or "distance in i direction", I guess
        rest_col = rest_arr[:,col_index]  # a 1D column. This is the piece of the onion we are on.
        normfac  = 1 / val1[idist,idist]

        for i in range(0,idist)[::-1]: 
            ic = w - i - 1 
            rest_arr[:,ic] =  rest_arr[:,ic] - rest_col * normfac * val1[i,idist]

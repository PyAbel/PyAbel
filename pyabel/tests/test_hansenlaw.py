...
def test_hansenlaw_shape():
    ...
    recon = pyabel.hansenlaw.hansenlaw_transform(x, direction='inverse')
...
def test_hansenlaw_forward_tansform_gaussian():
    ...
    ref = pyabel.tools.analytical.GaussianAnalytical(n,
                     r_max, symmetric=False,  sigma=200)

    recon = pyabel.hansenlaw.hansenlaw_transform(ref.func, ref.dr,
                                               direction='forward')
...
def test_hansenlaw_inverse_transform_gaussian():
    ...
    recon = pyabel.hansenlaw.hansenlaw_transform(tr, ref.dr, direction='inverse')
...
def test_hansenlaw_forward_curveA():
    ...
    n = 101
    curveA = pyabel.tools.analytical.TransformPair(n, profile=3)

    # forward Abel == g(r)
    Aproj = pyabel.hansenlaw.hansenlaw_transform(curveA.func, curveA.dr,
                                               direction='forward')
...
def test_hansenlaw_inverse_transform_curveA():
    ...
    # inverse Abel == f(r)
    recon = pyabel.hansenlaw.hansenlaw_transform(curveA.abel, curveA.dr,
                                               direction='inverse')
...
def test_hansenlaw_forward_dribinski_image():
    ...
    Q0, Q1, Q2, Q3 = pyabel.tools.symmetry.get_image_quadrants(IM)

    # forward Abel transform
    fQ0 = pyabel.hansenlaw.hansenlaw_transform(Q0, direction='forward')

    # inverse Abel transform
    ifQ0 = pyabel.hansenlaw.hansenlaw_transform(fQ0, direction='inverse')

    # speed distribution
    orig_speed, orig_radial = pyabel.tools.vmi.angular_integration_3D(Q0,
                              origin=(0, 0))

    speed, radial_coords = pyabel.tools.vmi.angular_integration_3D(ifQ0,
                           origin=(0, 0))
